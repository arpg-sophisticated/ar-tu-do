...
BruteforceReward::BruteforceReward()
{
    ...
    m_collision_detected_subscriber =
        m_node_handle.subscribe<std_msgs::Bool>(TOPIC_COLLISION_DETECTED, 1, &BruteforceReward::collisionCallback,
                                                this);
...
void BruteforceReward::controlledDriveParametersCallback(const drive_msgs::drive_param::ConstPtr& parameters)
{
    ...
    if (m_time_delta >= m_time_interval)
    {
        ...
        std::stringstream logline;
        logline << m_logentry << ";" << m_time_current << ";" << m_speed_current << ";" << m_average << ";"
                << m_speed_current_average_value << ";" << m_command << std::endl;
        filestream << logline.str();
        filestream.close();
        m_time_delta = ros::Duration(0.0f);
        m_logentry++;
...
void BruteforceReward::collisionCallback(const std_msgs::Bool::ConstPtr& collision)
{
    if (collision->data)
    {
        m_command = "abort";
