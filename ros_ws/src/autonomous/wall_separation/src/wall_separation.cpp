...
WallSeparation::WallSeparation()
  ...
  this->m_lidar_subscriber = m_node_handle.subscribe<sensor_msgs::LaserScan>(
      TOPIC_LASER_SCAN, 1, &WallSeparation::lidar_callback, this);
  this->m_voxel_publisher =
      m_node_handle.advertise<sensor_msgs::PointCloud2>(TOPIC_VOXEL_, 1, true);
}

std::vector<geometry_msgs::Point>
lidar_to_cartesian(const sensor_msgs::LaserScan::ConstPtr &lidar) {
  std::vector<geometry_msgs::Point> points;

  uint32_t scan_count =
      (lidar->angle_max - lidar->angle_min) / lidar->angle_increment;
...
void WallSeparation::lidar_callback(
  ...
  tmp3.name = 'c';
  tmp3.offset = 8;
  tmp3.datatype = sensor_msgs::PointField::UINT32;
  tmp3.count = 1;
  ...
  voxelsCloud.point_step =
      2 * sizeof(float) +
      sizeof(
          uint32_t); // FLOAT32 4Bytes * 2 (x und y Koordinate)+4 (count uint32)
  ...
  for (size_t cp = 0; cp < voxelsCloud.width; ++cp) {
    ...
    memcpy(&voxelsCloud.data[cp * voxelsCloud.point_step +
                             voxelsCloud.fields[0].offset],
           &voxels[cp]->x, sizeof(float));
    memcpy(&voxelsCloud.data[cp * voxelsCloud.point_step +
                             voxelsCloud.fields[1].offset],
           &voxels[cp]->y, sizeof(float));
    memcpy(&voxelsCloud.data[cp * voxelsCloud.point_step +
                             voxelsCloud.fields[2].offset],
           &tmp, sizeof(uint32_t));
  }
