#include "../include/rviz_geometry_publisher.h"

RvizGeometryPublisher::RvizGeometryPublisher(ros::NodeHandle node_handle,
                                             const std::string &topic,
                                             const std::string &frame)
    : m_frame(frame) {
  this->m_marker_publisher =
      node_handle.advertise<visualization_msgs::Marker>(topic, 10);
}

void RvizGeometryPublisher::drawLine(int id, geometry_msgs::Point point1,
...
void RvizGeometryPublisher::drawVoxels(int id,
                                       const PointCloud::ConstPtr &voxels,
  ...
  BOOST_FOREACH (const pcl::PointXYZ &pt, voxels->points) {

    rects.points.push_back(createPoint(pt.x, pt.y, 0.0f));
...
void RvizGeometryPublisher::drawRect(int id, double x, double y, double width,
                                     double height, float intensity = 1.0f) {
