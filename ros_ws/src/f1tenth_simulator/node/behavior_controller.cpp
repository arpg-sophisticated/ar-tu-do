...
class BehaviorController
{
    ...
    BehaviorController()
    {
        ...
        car_distances = Precompute::get_car_distances(scan_beams, wheelbase, width, scan_distance_to_base_link,
        ...
        if (state.velocity != 0)
        {
            for (size_t i = 0; i < msg.ranges.size(); i++)
            {
                ...
                if ((ttc < ttc_threshold) && (ttc >= 0.0))
                {
                    ...
                    collision_file << "Time since start of sim: " << (ros::Time::now().toSec() - beginning_seconds)
                                   << " seconds\n";
                    collision_file << "\n";
                    return;
                }
